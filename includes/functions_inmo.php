	...
	if ($registros > 0) {
		...
		while ($datos = $stmt->fetch()) {
			...
			$imagenes .= "<div " . ($cnt == 0 ? " class=\"item active\"" : " class=\"item\"") . ">" .
				"<img src=\"http://localhost/esure-main/modules_/imno/descargar_imagen.php?id_imagen=" . $datos["id"] . "&dataName=$_SESSION[db_name]\" style=\"width:" . ($datos["vertical"] == 1 ? 280 : 500) . "\">" .
				"</div>";
	...
	if ($registros > 0) {
		...
		while ($row = $stmt->fetch()) {
			...
			$arrayRta["cnt_NO_preferenciales"] .= "<div class=\"owl-item\" style=\"swidth: 240px;\">" .
				...
				"<img src=\"http://localhost/esure-main/modules_/imno/descargar_imagen.php?id_imagen=" . $id_imagen . "\" class=\"img-responsive\" alt=\"properties\">" .
	...
	if ($registros > 0) {
		...
		while ($row = $stmt->fetch()) {
			...
			if ($id_imagen > 0) {
				...
				$descripcion_01 = $row["dormitorios"] . " dormitorios -  $operacion";
				$comentario		 = $row["comentario"];
				$urlPublica  	 = "$_SESSION[INMO_URL]http://localhost/devs/proyectos/esecure_new/detalle_publico_propiedad.php?id=$row[id]";
				...
				$url  = "$_SESSION[INMO_URL]http://localhost/devs/proyectos/esecure_new/modules_/imno/detalle_publico_propiedad.php?id=$row[id]";
				//$arrayImg2 = array();
				//$sql2 ="SELECT id 
				//FROM inmo_imagenes
				...
				foreach ($id_imagen as $array => $id) {
					$arrayRta["cnt_NO_preferenciales"] .= "<div><img src=\"http://localhost/devs/proyectos/esecure_new/modules_/imno/descargar_imagen.php?id_imagen=" . $id["id"] . "&dataName=esure\" class=\"img-responsive\" style=\"max-height:250px;min-width:331px\" alt=\"properties\"/></div>";
				}
				...
				$arrayRta["cnt_NO_preferenciales"] .= "" .
					...
					"</div>" .
					"<a class=\"m-l-auto btn btn-esecure\" target=\"_blank\"  href=\"http://localhost/devs/proyectos/esecure_new/modules_/imno/detalle_publico_propiedad.php?id=$id_propiedad\"><span style=\"font-size:12px;\">Ver Detalles <span></a>" .
