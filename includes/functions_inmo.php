	...
	if ($registros > 0) {
		...
		while ($row = $stmt->fetch()) {
			...
			if ($id_imagen > 0) {
				...
				$descripcion_01 = $row["dormitorios"] . " dormitorios -  $operacion";
				$comentario		 = $row["comentario"];
				$urlPublica  	 = "$_SESSION[INMO_URL]http://localhost/esure-main/modules_/imno/detalle_publico_propiedad.php?id=$row[id]";
				...
				$url  = "$_SESSION[INMO_URL]http://localhost/esure-main/modules_/imno/detalle_publico_propiedad.php?id=$row[id]";
				//$arrayImg2 = array();
				//$sql2 ="SELECT id 
				//FROM inmo_imagenes
				...
				$arrayRta["cnt_NO_preferenciales"] .= "" .
					...
					"</div>" .
					"<a class=\"m-l-auto btn btn-esecure\" target=\"_blank\"  href=\"http://localhost/esure-main/modules_/imno/detalle_publico_propiedad.php?id=$id_propiedad\"><span style=\"font-size:12px;\">Ver Detalles <span></a>" .
